    ...
    <b-card v-for="product in products" :key="product.id" class="ecommerce-card" no-body>
      ...
      <b-card-body>
        ...
        <span class="delivery-date text-muted"
          >Delivery by
          {{ formatDate(product.shippingDate, { month: 'short', day: 'numeric', weekday: 'short' }) }}</span
        >
      ...
      <div class="item-options text-center">
        ...
        <b-button variant="primary" class="btn-cart move-cart" @click="toggleProductInWishlist(product)">
          <feather-icon icon="HeartIcon" class="mr-50" :class="{ 'fill-current': product.isInWishlist }" />
