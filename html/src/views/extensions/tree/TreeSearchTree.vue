...
export default {
  ...
  methods: {
    ...
    tpl(...args) {
      ...
      return (
        <span>
          <span class="tree-expand" onClick={() => this.$refs.tree.addNode(node, { title: 'sync node' })}>
            +
          </span>
          <span
            class={titleClass}
          ...
          <button class="btn-async text-warning border-0 cursor-pointer mr-1" onClick={() => this.asyncLoad(node)}>
            async
          </button>
          <button
