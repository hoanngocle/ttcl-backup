    ...
    <div id="knowledge-base-category">
      <b-row class="match-height">
        <b-col v-for="(data, index) in filteredKB" :key="index" md="4" sm="6">
          <b-card>
            <h6 class="kb-title">
              <feather-icon :icon="data.icon" size="20" class="mr-50" :class="data.iconColor" />{{ data.title }} ({{
    ...
    filteredKB() {
      ...
      return this.kb.filter(
        (item) =>
          ...
          item.questions.filter((queObj) => queObj.question.toLowerCase().includes(knowledgeBaseSearchQueryLower))
            .length,
      );
    },
  },
